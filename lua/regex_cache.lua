...
if not ngx.re.wafcache then
   ngx.log(ngx.ERR, "INIT WAF CACHE")
   ngx.re.wafcache = new_tab(10, 10)
end
...
local function cache_set(key, val)
   regex_match_cache[key] = val
end

local function cache_get(key)
   return regex_match_cache[key]
end
...
local function re_match_compile(regex, opts)
    ...
    if not regex_match_cache then
        ...
        if sz <= 0 then
           ...
           regex_match_cache = ngx.re.wafcache
        end
    end
    ...
    if compiled == nil then
       if compile_once then
          --ngx.log(ngx.ERR,"compiled regex not found, compiling regex...", key)
       end
        ...
        if compiled == nil then
           ngx.log(ngx.ERR, "compile failed ", key);
           cache_set(key, 0)
...
local function re_match_helper(subj, regex, opts, want_caps, res, nth)
    ...
    local rc = C.ngx_http_lua_ffi_exec_regex(compiled, flags, subj, #subj, 0)
